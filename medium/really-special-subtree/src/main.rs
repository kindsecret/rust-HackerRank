    ...
    for _ in 1..nodes {
        ...
        for &node in visited.iter() {
            for &next in connections[node].iter() {
                ...
                if min_weight > edges[node][next]
                    || (min_weight == edges[node][next] && min_node_sum > node + next)
                {
...
fn main() {
    ...
    let g_nodes_edges: Vec<String> = stdin_iterator
        .next()
        .unwrap()
        .unwrap()
        .split(' ')
    ...
    for _ in 0..g_edges {
        let g_from_to: Vec<String> = stdin_iterator
            .next()
            .unwrap()
            .unwrap()
            .split(' ')
