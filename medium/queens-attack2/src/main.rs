    ...
    for obstacle in obstacles.iter() {
        if obstacle[0] == r_q {
            ...
            if obstacle[0] > r_q && obstacle[1] > c_q {
                attacks[1] =
                    std::cmp::min(attacks[1], (obstacle[0] - r_q).min(obstacle[1] - c_q - 1));
            } else if obstacle[0] > r_q && obstacle[1] < c_q {
                attacks[3] =
                    std::cmp::min(attacks[3], (obstacle[0] - r_q).min(c_q - obstacle[1] - 1));
            } else if obstacle[0] < r_q && obstacle[1] < c_q {
                attacks[5] =
                    std::cmp::min(attacks[5], (r_q - obstacle[0]).min(c_q - obstacle[1] - 1));
            } else {
                attacks[7] =
                    std::cmp::min(attacks[7], (r_q - obstacle[0]).min(obstacle[1] - c_q - 1));
            }
        }
    }
